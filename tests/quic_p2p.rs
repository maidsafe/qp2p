...
fn test_peer_with_hcc(hard_coded_contacts: Vec<NodeInfo>) -> (QuicP2p, mpsc::Receiver<Event>) {
...
mod bootstrap {
    ...
    fn it_will_attempt_hard_coded_contacts() {
        ...
        let (mut peer2, ev_rx) = test_peer_with_hcc(vec![peer1_conn_info.clone()]);
    ...
    fn it_will_report_failure_when_bootstrap_cache_and_hard_coded_contacts_are_invalid() {
        ...
        let (mut peer, ev_rx) = test_peer_with_hcc(vec![dummy_peer_info]);
