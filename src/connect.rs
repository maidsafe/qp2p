...
impl QuicP2p {
    ...
    pub fn connect_to_node(
        ...
        let r = {
            ...
            let conn = self.connections.entry(node_addr).or_insert_with(|| {
                Connection::new(
                    node_addr,
                    event_tx.clone(),
            ...
            if conn.to_peer.is_no_connection() {
                ...
                if self.our_type == OurType::Client {
                    if !conn.from_peer.is_no_connection() {
                        panic!(
                            "Logic Error - cannot expect Network to reverse connect to a client"
                        );
                    }
                ...
                let connecting = self.quic_ep().connect_with(
                ...
                let _ = tokio::spawn(async move {
                    select! {
                        ...
                        new_peer_conn_res = connecting.fuse() => {
                            self.handle_new_connection_res(
                                node_addr,
                                new_peer_conn_res
                            );
                        },
        ...
        if let Err(e) = r.as_ref() {
            self.handle_connect_err(node_addr, e);
        }
    ...
    async fn handle_new_connection_res(
        ...
        let (q_conn, uni_streams, bi_streams) = match new_peer_conn_res {
            ...
            Err(e) => return self.handle_connect_err(peer_addr, &From::from(e)),
        };
        ...
        if conn.we_contacted_peer {
            self.bootstrap_cache.add_peer(peer_addr);
        }
        ...
        for (pending_send, token) in pending_sends {
            self.write_to_peer_connection(Peer::Node(peer_addr), &q_conn, pending_send, token);
        }
    ...
    fn handle_connect_err(&self, peer_addr: SocketAddr, e: &QuicP2pError) {
        debug!(
